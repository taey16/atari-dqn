...
local agent_params = 'lr=0.00025,eq=1,ep_end=0.1,ep_endt=replay_memory,discount=0.99,hist_len=4,learn_start=50000,replay_memory=1000000,update_freq=4,n_replay=1,network=\"convnet_atari3\",preproc=\"net_downsample_2x_full_y\",state_dim=7056,minibatch_size=32,rescale_r=1,ncols=1,bufferSize=512,valid_size=500,target_q=10000,clip_delta=1,min_reward=-1,max_reward=1'

local checkpoint_agent_name = ('DQN3_0_1_%s_FULL_Y'):format(env_name)
...
cmd:option('-prog_freq', 12500, 'frequency of progress output')
...
print('===> Saving everything to: ' .. opt.checkpoint_path)
io.flush()
