...
function nql:compute_validation_statistics()
  local targets, delta, q2_max = 
    self:getQUpdate{s=self.valid_s, a=self.valid_a, r=self.valid_r, s2=self.valid_s2, term=self.valid_term}
...
function nql:perceive(reward, rawstate, terminal, testing, testing_ep)
  ...
  if not terminal then
    actionIndex, exploration = self:eGreedy(curState, testing_ep)
  end
  ...
  if not terminal then
    return actionIndex, exploration
  else
    return 0, -1
  end
end
...
function nql:eGreedy(state, testing_ep)
  ...
  if torch.uniform() < self.ep then
    return torch.random(1, self.n_actions), 0
  else
    return self:greedy(state), 1
  end
end
